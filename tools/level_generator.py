...
from word_dictionary import EnglishDictionary, TrieNode
...
class LevelGenerator:
    def __init__(self, dictionary: EnglishDictionary):
        ...
        self.common_set = set(self.dict.common_words)
    ...
    def generate_single_level(
        ...
        """Generate a validated, solvable level with REAL, MEANINGFUL words."""
        rng = random.Random(level_id * 104729 + 17)

        # Scale vocabulary pool based on progression (Top common words)
        if level_id <= 30:
            allowed_pool_size = 800
        elif level_id <= 100:
            allowed_pool_size = 1800
        elif level_id <= 300:
            allowed_pool_size = 3500
        elif level_id <= 1000:
            allowed_pool_size = 6000
        else:
            allowed_pool_size = len(self.dict.common_words)

        allowed_target_words_set = set(self.dict.common_words[:allowed_pool_size])
        ...
        for attempt in range(max_attempts):
            ...
            # 3. Select Target Words: strictly common, meaningful everyday words
            ...
            # 4. Extra Words: other valid English words from Enable1 dictionary
    ...
    def _generate_filled_grid(
        ...
        word_pool = [w for w in self.dict.common_words if w in allowed_set]

        # 1st anchor word
        anchor_len = min(max_len, total_cells)
        anchor_candidates = [
            w for w in word_pool if min_len <= len(w) <= anchor_len and any(v in w for v in "AEIOU")
        ]
        ...
        for _ in range(30):
            ...
            cand_words = [w for w in word_pool if len(w) == target_len and any(v in w for v in "AEIOU")]
            if not cand_words:
                continue
        ...
        # Fill any stubborn isolated empty cells
        vowels = ["A", "E", "I", "O", "U"]
        consonants = ["R", "S", "T", "L", "N", "D", "M", "P", "C", "B"]
        for r in range(height):
            for c in range(width):
                if grid[r][c] == "":
    ...
    def _select_target_words(
        ...
        # Filter candidates: strictly in allowed_set and has vowel
        candidates = [
            w for w in all_found.keys()
            if w in allowed_set and min_word_len <= len(w) <= max_word_len and any(v in w for v in "AEIOUY")
        ]

        if len(candidates) < min_targets:
            return None

        # Sort candidate words by common frequency rank
        candidates.sort(key=lambda w: self.dict.common_words.index(w) if w in self.common_set else 999999)
    ...
    def _validate_solvable(self, grid: List[List[str]], target_words: List[str], obstacles: List[dict]) -> bool:
        """Simulate solving the level to ensure 100% solvability."""
        height = len(grid)
        width = len(grid[0])
        ...
        for w in target_words:
            ...
            for r in range(height):
                for c in range(width):
                    if grid[r][c] == w[0]:
                        if self._can_trace_word(grid, w, 1, r, c, {(r, c)}):
                            found = True
                            break
                if found:
                    break
    ...
    def _can_trace_word(
        self, grid: List[List[str]], word: str, idx: int, r: int, c: int, visited: Set[Tuple[int, int]]
    ) -> bool:
        if idx >= len(word):
            return True
        ...
        for dr, dc in DIRECTIONS:
            ...
            if 0 <= nr < height and 0 <= nc < width and (nr, nc) not in visited and grid[nr][nc] == char:
                visited.add((nr, nc))
                if self._can_trace_word(grid, word, idx + 1, nr, nc, visited):
                    return True
